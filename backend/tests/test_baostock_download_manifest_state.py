...
from trading_codex.baostock_download.cli import _run
from trading_codex.baostock_download.constants import (
    BLACKLIST_RULES_SHA256,
    provider_rule_snapshot,
)
from trading_codex.baostock_download.download_cli import _parser
...
def test_cli_has_one_storage_location_parameter(tmp_path: Path) -> None:
    ...
    args = parser.parse_args(
        ["--data-root", str(data_root), "--requests", str(requests)]
    )

    assert args.data_root == data_root
    assert args.requests == str(requests)
    ...
    with pytest.raises(SystemExit):
        parser.parse_args(["doctor", "--initialize", "--json"])
