...
from trading_codex.baostock_download.manifest import ManifestItem
from trading_codex.data.models import ProviderBatch


class BaoStockProvider:
    """Thin endpoint adapter used only by the fetch command."""
    ...
    def fetch(self, item: ManifestItem) -> ProviderBatch:
        if not self.logged_in:
    ...
    def _query(self, item: ManifestItem, query: dict[str, str]) -> Any:
        method = getattr(self.module, item.endpoint)
    ...
    def _collect(self, item: ManifestItem, result: Any) -> ProviderBatch:
