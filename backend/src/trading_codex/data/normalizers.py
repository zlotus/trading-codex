...
def normalize_daily_bars(batch: ProviderBatch, artifact: RawArtifact) -> list[NormalizedRow]:
    ...
    for row in batch.rows:
        ...
        volume = _daily_volume(row, trading=trade_status)
        ...
        normalized.append(
            {
                ...
                **prices,
                "volume": volume,
                "amount": amount,
                "adjustment_flag": _adjustment_flag(batch, row),
...
def _daily_volume(row: dict[str, str], *, trading: bool) -> int:
    value = row.get("volume")
    if value == "" and not trading:
        return 0
    return _integer(_required(row, "volume"), field="volume")


def _adjustment_flag(batch: ProviderBatch, row: dict[str, str]) -> str:
    actual = _required(row, "adjustflag")
    if actual not in {"1", "2", "3"}:
        raise DataValidationError(f"invalid adjustment flag: {actual!r}")
    expected = batch.query.get("adjustflag")
    if expected is None:
        return actual
    if expected not in {"1", "2", "3"}:
        raise DataValidationError(f"invalid exact query adjustment flag: {expected!r}")
    if actual == expected:
        return expected
    # BaoStock labels an unadjusted prefix of some forward-adjusted series as 3.
    if expected == "2" and actual == "3":
        return expected
    raise DataValidationError(
        f"provider adjustment flag {actual!r} differs from exact query {expected!r}"
    )
